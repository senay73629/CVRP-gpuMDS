...
// - Search radius doubles each iteration until full coverage (progressive fallback)
...
// MODIFIED: v4 — KNN-based cheapest cross-component edge finder using spatial hash grid
    ...
    int n)
{
    ...
    // Compute range of cells to search
    ...
    // MODIFIED: v4 — If no cross-component neighbor found, set weight to FLT_MAX sentinel
...
void BoruvkaMST(const VRP& vrp, int N,
  ...
  // ======================================================================
  // MODIFIED: v4 — Step 4: Borůvka loop with KNN on first iterations
  //                        and progressive radius expansion
  // ======================================================================
  int phase = 0;
  int h_mst_count = 0;

  // MODIFIED: v4 — Initialize search radius and fallback control
  float search_radius = cell_size;         // start with one cell width
  float max_radius = sqrtf(               // max possible distance in bounding box
  ...
  while (h_mst_count < N - 1) {
    ...
    // MODIFIED: v4 — Choose between KNN hash grid kernel and full O(n²) scan
    if (use_knn) {
        // Use KNN hash grid kernel
        findCheapestEdgeKNN<<<GRD_BORUVKA, BLK_BORUVKA>>>(
            d_x, d_y, d_comp,
        ...
        // Check if any point got FLT_MAX (no cross-component neighbor found in radius)
        bool any_missed = checkMissedEdges(d_cheapest_w, N);
        if (any_missed) {
            search_radius *= 2.0f;
            // Clamp to max_radius to prevent Inf/NaN
            if (search_radius > max_radius) search_radius = max_radius;
            if (search_radius >= max_radius) {
                use_knn = false; // radius covers full space, switch to full scan permanently
            }
            if (g_verbose) std::cerr << "v4: KNN missed edges, expanding radius to "
                      << search_radius << " (max=" << max_radius << ")\n";
            continue; // retry this iteration with expanded radius
        }
        ...
        if (search_radius > max_radius) search_radius = max_radius;
        if (search_radius >= max_radius) {
            use_knn = false; // next iteration will use full scan
        }
...
int main(int argc, char* argv[]) {
  ...
  std::ofstream ofs("v4.txt", std::ios::app);
  if (ofs.is_open()) {
    ofs << argv[1]
        << "\tMinCost: " << minCost
        << "\tTimeMST: " << time_mst
