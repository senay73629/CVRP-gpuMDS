...
//   d_cheapest_from[u] — u itself (convenience for merge step)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// CPU-side Union-Find (Disjoint Set Union) with path compression + union by rank
// Used in the CPU merge step after each Borůvka kernel phase.
// -----------------------------------------------------------------------
struct DSU {
  ...
  // Returns true if a merge actually happened (u and v were in different sets)
...
// -----------------------------------------------------------------------
// BoruvkaMST — main driver
//
// Runs Borůvka's algorithm using GPU kernels for cheapest-edge finding
// and CPU for component merging via Union-Find.
//
// Returns an MST adjacency list in the same format as PrimsAlgo():
//   std::vector<std::vector<Edge>>  nG  (bi-directional)
//
// Parameters:
//   vrp       — VRP instance (for node coordinates and toRound flag)
//   N         — number of nodes
// -----------------------------------------------------------------------
std::vector<std::vector<Edge>> BoruvkaMST(const VRP& vrp, int N) {
  // ---- INITIALIZATION ----
  // Component labels: each node starts as its own component
  std::vector<int> h_comp(N);
  for (int i = 0; i < N; ++i) h_comp[i] = i;
  ...
  // Host buffers for cheapest edges (copied back after each kernel)
  std::vector<int>    h_cheapest_to(N);
  std::vector<double> h_cheapest_w(N);

  // MST adjacency list (output)
  std::vector<std::vector<Edge>> mstG(N);
  int mst_edge_count = 0;

  // DSU for component merging on CPU
  DSU dsu(N);

  const int BLK_BORUVKA = 16;
  const int GRD_BORUVKA = (N + BLK_BORUVKA - 1) / BLK_BORUVKA;

  int phase = 0;

  // ---- BORŮVKA MAIN LOOP ----
  // Repeat until MST has exactly N-1 edges (i.e., one component remains)
  while (mst_edge_count < N - 1) {
    phase++;

    // ---- PHASE: Upload current component labels to GPU ----
    CUDA_CHECK(110, cudaMemcpy(d_comp, h_comp.data(), N*sizeof(int), cudaMemcpyHostToDevice));
    ...
    boruvkaFindCheapest<<<GRD_BORUVKA, BLK_BORUVKA>>>(
        ...
        N);
    CUDA_CHECK(111, cudaGetLastError());
    CUDA_CHECK(112, cudaDeviceSynchronize());

    // ---- PHASE: Copy results back to host ----
    CUDA_CHECK(113, cudaMemcpy(h_cheapest_to.data(), d_cheapest_to, N*sizeof(int),    cudaMemcpyDeviceToHost));
    CUDA_CHECK(114, cudaMemcpy(h_cheapest_w.data(),  d_cheapest_w,  N*sizeof(double), cudaMemcpyDeviceToHost));

    // ---- PHASE: Component merging on CPU ----
    // For each component, identify the cheapest outgoing edge among all its member nodes.
    // Then merge components and add selected edges to MST.

    // Map: component root → (best_u, best_v, best_w) — cheapest edge for that component
    // We iterate all nodes, track per-component best, then merge.
    struct BestEdge { int u, v; double w; };
    std::vector<BestEdge> compBest(N, {-1, -1, DBL_MAX});

    for (int u = 0; u < N; ++u) {
      int v = h_cheapest_to[u];
      if (v < 0) continue;                          // no cross-component neighbor
      double w = h_cheapest_w[u];

      int cu = dsu.find(u);                         // component root of u
      if (w < compBest[cu].w) {
        compBest[cu] = {u, v, w};
      }
    }

    // ---- PHASE: Add selected edges and merge components ----
    bool any_merged = false;
    for (int c = 0; c < N; ++c) {
      if (compBest[c].u < 0) continue;             // component c has no outgoing edge
      int u = compBest[c].u;
      int v = compBest[c].v;
      double w = compBest[c].w;

      if (dsu.unite(u, v)) {                        // merge if still in different components
        // Round weight to match VRP convention
        weight_t ew = vrp.toRound ? std::round(w) : w;
        mstG[u].push_back(Edge(v, ew));
        mstG[v].push_back(Edge(u, ew));
        mst_edge_count++;
        any_merged = true;
      }
    }
    ...
    // ---- TERMINATION CHECK ----
    if (!any_merged) {
      // No merges occurred — graph may have multiple connected components
      // (shouldn't happen for complete Euclidean graphs, but guard anyway)
      std::cerr << "Borůvka: no merges in phase " << phase
                << " — graph may be disconnected. MST edges: " << mst_edge_count << "\n";
      break;
    }

    // ---- PHASE: Propagate updated component labels (flatten DSU for GPU) ----
    for (int i = 0; i < N; ++i) h_comp[i] = dsu.find(i);

    std::cerr << "Borůvka phase " << phase
              << ": MST edges so far = " << mst_edge_count << "\n";
  }

  std::cerr << "Borůvka complete: " << mst_edge_count
            << " edges in " << phase << " phases.\n";

  // ---- CLEANUP GPU BUFFERS (Borůvka-specific) ----
  CUDA_CHECK(120, cudaFree(d_x));
  CUDA_CHECK(121, cudaFree(d_y));
  CUDA_CHECK(122, cudaFree(d_comp));
  CUDA_CHECK(123, cudaFree(d_cheapest_to));
  CUDA_CHECK(124, cudaFree(d_cheapest_w));
...
int main(int argc, char* argv[]) {
  ...
  // Run Borůvka's parallel MST — returns MST as adjacency list
  auto mstG = BoruvkaMST(vrp, N);

  // Convert MST adj list → CSR for GPU (Part 2)
  CSR mst    = adjListToCSR(mstG);
  int mst_nnz = (int)mst.col_idx.size();  // 2*(N-1)
  std::cerr << "MST edges (Borůvka): " << mst_nnz/2 << "\n";
  ...
  CUDA_CHECK(7, cudaMalloc(&d_csr_row, (N+1)    * sizeof(int)));
  CUDA_CHECK(8, cudaMalloc(&d_csr_col,  mst_nnz * sizeof(int)));
  CUDA_CHECK(9, cudaMemcpy(d_csr_row, mst.row_ptr.data(), (N+1)*sizeof(int),    cudaMemcpyHostToDevice));
  CUDA_CHECK(10, cudaMemcpy(d_csr_col, mst.col_idx.data(), mst_nnz*sizeof(int),  cudaMemcpyHostToDevice));
...
//   CUDA_CHECK(cudaDeviceSynchronize());
  ...
  //           << " " << time_total
  //           << (valid ? " VALID" : " INVALID") << "\n";
  std::ofstream ofs("v3.txt", std::ios::app);
  if (ofs.is_open()) {
    ofs << argv[1]
        << "\tMinCost: " << minCost
        << "\tTimeMST: " << time_mst
